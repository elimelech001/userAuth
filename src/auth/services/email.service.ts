...
export class AuthEmailService {
  ...
  async sendEmailVerification(userIdentity: UserIdentity) {
    ...
    const emailTemplate: EmailTemplate = this.emailTemplateService.generateVerificationEmail(token.access_token);

    await this.emailService.sendEmail('a0583234663@gmail.com', emailTemplate.subject, emailTemplate.content); //(await user).email
